const i18n = {
    cs: {
        ...
        siteTitle: "Error Code Explorer",
        siteSubtitle: "Vyhledej řešení pro svůj problémový kód"
    },
    en: {
        ...
        siteTitle: "Error Code Explorer",
        siteSubtitle: "Find solutions for your error codes"
    },
    zh: {
...
const langSelect = document.getElementById('langSwitcher'); // mapping to new ID for compatibility if needed, but let's be clean
...
function renderDashboard() {
    ...
    featured.forEach((err, index) => {
        const tile = document.createElement('div');
        tile.className = `dash-tile ${err.type} reveal`;
        tile.style.transitionDelay = `${index * 80}ms`;
        ...
        // Interactive mouse hover glow effect
...
function loadMoreCards() {
    ...
    const fragment = document.createDocumentFragment();
    batch.forEach((err, index) => {
        const card = buildCard(err, t);
        card.style.transitionDelay = `${(index % RENDER_BATCH) * 30}ms`;
...
function buildCard(err, t) {
    ...
    card.className = `card ${err.type} reveal`;
    const cat = err.category[currentLang] || err.category['en'] || '';

    // Truncate description for better card layout
    let desc = err.description[currentLang] || err.description['en'] || '';
    const MAX_DESC = 140;
    if (desc.length > MAX_DESC) desc = desc.substring(0, MAX_DESC) + '...';

    // Highlight search term
    const highlightedTitle = highlightText(err.code, query);
    const highlightedDesc = highlightText(desc, query);

    // Extract first step or truncate solution
    let solRaw = err.solution[currentLang] || err.solution['en'] || '';
    let solSnippet = solRaw;
    if (solRaw.startsWith('<ol')) {
        const match = solRaw.match(/<li>(.*?)<\/li>/);
        solSnippet = match ? match[1].replace(/<[^>]*>/g, '') : '';
    } else {
        solSnippet = solRaw.replace(/<[^>]*>/g, '');
    }
    const MAX_SOL = 100;
    if (solSnippet.length > MAX_SOL) solSnippet = solSnippet.substring(0, MAX_SOL) + '...';
    ...
    card.innerHTML = `
        <div class="card-header">
            <span class="badge ${err.type}">${err.type.toUpperCase()}</span>
            <span class="category">${cat}</span>
        </div>
        <h3 class="code-title">${highlightedTitle}</h3>
        <div class="card-section">
            <p class="description"><strong>${t.modalDescTitle}:</strong> ${highlightedDesc}</p>
        </div>
        ${vHtml}
        <div class="solution">
        ...
        <div class="card-footer-hint">
            <span>Zobrazit detailní řešení</span>
...
function openModal(err) {
    ...
    // Solution – supports rich HTML (ol.sol-steps) or plain text
    const solRaw = err.solution[currentLang] || '';
    if (Array.isArray(err.solutionSteps && err.solutionSteps[currentLang])) {
        // Reserved for future typed steps
        modalSolution.textContent = solRaw;
    } else if (typeof solRaw === 'string' && solRaw.startsWith('<')) {
        modalSolution.innerHTML = solRaw;
    } else {
        // Plain text – wrap in <p> for consistent styling
        modalSolution.innerHTML = `<p>${solRaw}</p>`;
    }

    // YouTube tutorial links
    const ytSection = document.getElementById('modalYtSection');
    const ytLinks = document.getElementById('modalYtLinks');
    if (err.youtube && err.youtube.length > 0) {
        ytLinks.innerHTML = err.youtube.map(yt =>
            `<a class="yt-link" href="${yt.url}" target="_blank" rel="noopener noreferrer">
                <span class="yt-link-icon"><i class="fa-solid fa-play"></i></span>
                <span>${yt.title}</span>
                <span class="yt-link-arrow">↗</span>
            </a>`
        ).join('');
...
function updateUI() {
    ...
    if (uiElements.swWin) uiElements.swWin.textContent = t.swWindows;
    if (uiElements.swBios) uiElements.swBios.textContent = t.swBios;
    if (uiElements.swNet) uiElements.swNet.textContent = t.swNetwork;
    ...
    if (uiElements.siteTitle) uiElements.siteTitle.textContent = t.siteTitle;
    if (uiElements.siteSubtitle) uiElements.siteSubtitle.textContent = t.siteSubtitle;
}

// =========================================================
// Custom Language Dropdown Logic
// =========================================================
const langSelectedTrigger = document.getElementById('langSelected');
const currentLangText = document.getElementById('currentLangText');
const langOptionItems = document.querySelectorAll('.lang-option');

if (langSelectedTrigger) {
    langSelectedTrigger.addEventListener('click', (e) => {
        e.stopPropagation();
...
langOptionItems.forEach(opt => {
    opt.addEventListener('click', () => {
        ...
        langOptionItems.forEach(o => o.classList.toggle('active', o === opt));
...
document.querySelectorAll('.tab').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (activeType === 'all') {
            ...
            filtersCollapsible.classList.remove('open');
...
// Initialize mouse hover glow effect for hardcoded dashboard tiles
...
function handleChat() {
    ...
    setTimeout(() => {
        ...
        // Intelligent Keyword / Code Matching
        foundMatch = errorCodes.find(err =>
            lowerText.includes(err.code.toLowerCase()) ||
            lowerText.includes(err.id.toLowerCase()) ||
            (err.vendors && err.vendors.some(v => lowerText.includes(v.toLowerCase())))
        );
        ...
        if (foundMatch) {
            const desc = foundMatch.description[currentLang] || foundMatch.description['en'] || "";
            if (isEn) {
                response = `I understand. For the code **${foundMatch.code}**, I found the following in our database:\n\n*${desc}*\n\nWould you like me to show you the complete solution?`;
            } else if (isZh) {
                response = `我明白了。对于代码 **${foundMatch.code}**，我在数据库中找到了以下信息：\n\n*${desc}*\n\n你想让我展示完整的解决方案吗？`;
            } else {
                response = `Rozumím. Pro kód **${foundMatch.code}** jsem v naší databázi našel následující informace: \n\n*${desc}*\n\nChceš, abych ti zobrazil kompletní podrobný návod k opravě?`;
            }
            ...
            btn.innerHTML = isEn ? '<i class="fa-solid fa-wrench"></i> Show Solution' : isZh ? '<i class="fa-solid fa-wrench"></i> 显示解决方案' : '<i class="fa-solid fa-wrench"></i> Zobrazit řešení';
            btn.onclick = () => {
                openById(foundMatch.id);
                if (isEn) addChatMessage(`Opening details for ${foundMatch.code}. Hope this helps!`, 'ai');
                else if (isZh) addChatMessage(`正在打开 ${foundMatch.code} 的详细信息。希望对你有帮助！`, 'ai');
                else addChatMessage(`Otevírám detail pro ${foundMatch.code}. Doufám, že ti to pomůže!`, 'ai');
            ...
            // Conversational fallback based on components
            if (lowerText.includes('gpu') || lowerText.includes('grafik') || lowerText.includes('obraz') || lowerText.includes('video') || lowerText.includes('display')) {
                if (isEn) response = "It looks like a graphics issue. If you see artifacts or low performance, check drivers or search 'Code 43'. Can I look up something specific?";
                else if (isZh) response = "看起来是图形问题。如果你看到伪影或性能低下，请检查驱动程序或搜索 'Code 43'。我可以查找具体内容吗？";
                else response = "Vypadá to na potíže s grafickým výstupem. Pokud vidíš artefakty nebo máš nízký výkon, doporučuji začít kontrolou ovladačů nebo hledáním 'Code 43'.";
            } else if (lowerText.includes('ram') || lowerText.includes('paměť') || lowerText.includes('bsod') || lowerText.includes('memory')) {
                if (isEn) response = "RAM issues are a common cause of BSODs. Check out 'IRQL_NOT_LESS_OR_EQUAL'. Want me to find it?";
                else if (isZh) response = "内存问题是导致蓝屏的常见原因。查看 'IRQL_NOT_LESS_OR_EQUAL'。需要我找出来吗？";
                else response = "Chyby paměti RAM jsou nejčastější příčinou modrých smrtí (BSOD). Doporučuji vyhledat kód 'IRQL_NOT_LESS_OR_EQUAL'.";
            } else if (lowerText.includes('disk') || lowerText.includes('ssd') || lowerText.includes('pomal') || lowerText.includes('slow')) {
                if (isEn) response = "For disk issues, checking S.M.A.R.T. status is key. Do you have a specific Windows error code?";
                else if (isZh) response = "对于磁盘问题，检查 S.M.A.R.T. 状态是关键。你有具体的 Windows 错误代码吗？";
                else response = "U problémů s diskem je nejdůležitější zkontrolovat S.M.A.R.T. status. Máš nějaký konkrétní kód?";
            } else if (lowerText.includes('update') || lowerText.includes('aktualiz')) {
                if (isEn) response = "Windows Update can be stubborn. A common error is '0x80070643'. Is that what you see?";
                else if (isZh) response = "Windows 更新有时很顽固。常见的错误是 '0x80070643'。你看到的是这个吗？";
                else response = "Windows Update dokáže být občas tvrdohlavý. Nejčastější chybou je '0x80070643'. Je to tvůj případ?";
            } else if (lowerText.match(/\b(ahoj|čau|zdravím|hello|hi|你好)\b/)) {
                if (isEn) response = "Hi! I'm Antigravity AI. I'm here to help you decode PC errors. How can I assist today?";
                else if (isZh) response = "你好！我是 Antigravity AI。我是来帮你解码电脑错误的。今天我能怎么帮你？";
                else response = "Zdravím! Jsem Antigravity AI. Jsem tu, abych ti pomohl rozšifrovat chybové kódy v tvém PC. Jak můžu pomoci?";
            } else {
                if (isEn) response = "Sorry, I couldn't find an exact match for that. Please try entering a specific error code (e.g., 0x800...) or describe the failing component.";
                else if (isZh) response = "抱歉，我没有找到完全匹配的项。请尝试输入特定的错误代码或描述出现故障的组件。";
                else response = "Omlouvám se, ale pro tento dotaz jsem nenašel shodu. Zkus zadat přesný kód chyby nebo popsat komponentu, která nefunguje.";
            }
            addChatMessage(response, 'ai');
        }
        chatMessages.scrollTop = chatMessages.scrollHeight;
...
const revealObserver = hasIO ? new IntersectionObserver((entries) => {
...
}, { threshold: 0.05, rootMargin: '0px 0px -50px 0px' }) : null;
...
setTimeout(() => {
    ...
    // Failsafe: Force reveal all elements after a delay if they are still hidden
    setTimeout(() => {
