...
const errorCodes = [
    ...
    {
        id: "net-packet-loss", type: "software", subcategory: "network",
        code: "High Packet Loss / Ping Spikes in Online Games",
        category: { cs: "Síť", en: "Network", zh: "网络" },
        description: {
            cs: "Online hry nebo VoIP mají vysoké latence nebo ztrátu paketů – problém s routerem, ISP nebo QoS.",
            en: "Online games or VoIP experience high latency or packet loss – router, ISP, or QoS issue.",
            zh: "在线游戏或 VoIP 出现高延迟或丢包——路由器、ISP 或 QoS 问题。"
        },
        solution: {
            cs: "Přejděte na kabelové připojení. Aktivujte QoS v routeru. Kontaktujte ISP při konzistentním packet loss.",
            en: "Switch to wired connection. Enable QoS in router for gaming traffic. Contact ISP for persistent packet loss.",
            zh: "切换到有线连接，在路由器中为游戏流量启用 QoS，对于持续丢包联系 ISP。"
        },
        details: {
            cs: "Packet loss a latence jsou odlišné problémy: Latence (ping) = doba cesty paketu od zdrojů k cíli a zpět (RTT). Packet loss = procento paketů, které nedorazí k cíli. Diagnostika: 'ping -n 100 8.8.8.8' zobrazí RTT statistiky a ztrátu; 'tracert 8.8.8.8' identifikuje problematický hop (skok) v routovací cestě. WinMTR (Windows My TraceRoute) kombinuje ping a tracert v reálném čase. Příčiny packet loss: (1) Wi-Fi interference – 2.4 GHz přetíženo, přejděte na 5 GHz nebo 6 GHz (Wi-Fi 6E). (2) Router buffer bloat – příliš velká fronta v routeru způsobuje vysoké latence. Řešení: aktivujte Smart Queue Management (SQM) nebo CAKE algoritmus na routeru (OpenWrt). (3) ISP peering problémy – ztráta paketů mezi ISP uzly. (4) NIC driver interrupt coalescing – Windows nastaví adaptér méně citlivý na pakety. (5) TCP vs UDP: hry používají UDP, který nemá retransmission – každý ztracený paket = lag spike.",
            en: "Packet loss and latency are distinct issues. Latency (ping) = round-trip time of packets (RTT). Packet loss = percentage of packets that never reach the destination. Diagnostics: 'ping -n 100 8.8.8.8' shows RTT statistics and loss; 'tracert 8.8.8.8' identifies the problematic hop. WinMTR (Windows My TraceRoute) combines both in real time. Packet loss causes: (1) Wi-Fi interference – 2.4GHz congestion; switch to 5GHz or 6GHz (Wi-Fi 6E). (2) Router buffer bloat – excessive router queue causes high latency; enable SQM or CAKE algorithm (OpenWrt). (3) ISP peering issues – loss between ISP nodes. (4) NIC driver interrupt coalescing – Windows configures the adapter to be less packet-responsive. (5) TCP vs UDP: games use UDP, which has no retransmission – each lost packet = lag spike.",
            zh: "丢包和延迟是不同的问题。延迟（ping）= 数据包往返时间（RTT）。丢包 = 未到达目的地的数据包百分比。诊断：'ping -n 100 8.8.8.8' 显示 RTT 统计和丢包率；'tracert' 识别问题跳点；WinMTR 实时组合两者。丢包原因：(1) Wi-Fi 干扰——2.4GHz 拥塞，切换到 5GHz 或 6GHz；(2) 路由器缓冲区膨胀——启用 SQM 或 CAKE 算法；(3) ISP 对等互联问题；(4) 网卡驱动中断聚合；(5) 游戏使用 UDP，每个丢失的数据包 = 延迟峰值。"
        }
    },
    {
        ...
        category: { cs: "Síť", en: "Network", zh: "网络" },
        description: {
            cs: "Aplikace (Xbox, Teams) nefungují správně – problém s IPv6 nebo Teredo tunelem.",
            en: "Applications (Xbox, Teams) malfunction – IPv6 configuration or Teredo tunnel issue.",
            zh: "应用程序（Xbox、Teams）功能异常——IPv6 配置或 Teredo 隧道问题。"
        },
        solution: {
            cs: "Resetujte Teredo: 'netsh interface teredo set state disable' a poté 'type=default'. Povolte IPv6 na adaptéru.",
            en: "Reset Teredo: 'netsh interface teredo set state disable' then 'type=default'. Enable IPv6 on network adapter.",
            zh: "重置 Teredo：'netsh interface teredo set state disable' 然后'type=default'，在网络适配器上启用 IPv6。"
        },
        details: {
            cs: "IPv6 je nová verze Internet Protocol poskytující obrovský adresní prostor (128-bit vs 32-bit IPv4). Teredo je IPv6 tunelování přes UDP/IPv4 NAT – Microsoft implementoval Teredo pro Xbox online services a P2P aplikace, které vyžadují end-to-end IPv6 konektivitu. Příčiny problémů: (1) ISP blokuje Teredo server UDP port 3544 – zjistěte 'netsh interface teredo show state'. (2) VPN deaktivoval nebo přepsal Teredo adaptér. (3) Router nastavení – pokud router podporuje IPv6, Teredo by neměl být potřeba (native dual-stack). (4) Windows Firewall blokuje IPv6. (5) Skupina pravidel Xbox/Teams závisí na IPv6 multicast. Alternativy: 6to4 (méně spolehlivé), native ISP IPv6 dual-stack. Ověření: 'ping -6 ipv6.google.com'; 'Test Xbox Connectivity' v Xbox app zobrazí IPv6 status.",
            en: "IPv6 is the newer Internet Protocol version with a massive address space (128-bit vs 32-bit IPv4). Teredo is IPv6 tunneling over UDP/IPv4 NAT – Microsoft implemented Teredo for Xbox online services and P2P apps requiring end-to-end IPv6. Problem causes: (1) ISP blocks Teredo server UDP port 3544 – check with 'netsh interface teredo show state'. (2) VPN disabled or overrode the Teredo adapter. (3) Router settings – if the router supports native IPv6, Teredo is not needed (dual-stack). (4) Windows Firewall blocking IPv6. (5) Xbox/Teams rely on IPv6 multicast. Alternatives: 6to4 (less reliable), native ISP IPv6 dual-stack. Verification: 'ping -6 ipv6.google.com'; Xbox app → Test Xbox Connectivity shows IPv6 status.",
            zh: "IPv6 是新版本互联网协议，具有巨大的地址空间（128 位 vs 32 位 IPv4）。Teredo 是通过 UDP/IPv4 NAT 的 IPv6 隧道——微软为 Xbox 在线服务和需要端到端 IPv6 的 P2P 应用实现了 Teredo。问题原因：(1) ISP 阻止 Teredo 服务器 UDP 端口 3544；(2) VPN 禁用或覆盖了 Teredo 适配器；(3) 路由器设置——若路由器支持原生 IPv6，则不需要 Teredo；(4) Windows 防火墙阻止 IPv6；(5) Xbox/Teams 依赖 IPv6 组播。验证：'ping -6 ipv6.google.com'。"
        }
    },
...
const FEATURED_CODES = ['gpu-43', 'cpu-whea', 'ram-irql', 'win-bsod-critical', 'net-dns', 'disk-smart', 'app-vcredist', 'win-registry'];
...
function renderDashboard() {
    ...
    featured.forEach(err => {
        ...
        tile.innerHTML = `
            ...
            <div class="dash-tile-badge badge ${err.type}">${err.type.toUpperCase()}</div>
        `;
...
function renderCards() {
    ...
    if (final.length === 0) {
        ...
        requestAnimationFrame(() => resultsContainer.innerHTML = '');
    } else {
...
function updateUI() {
    ...
    document.getElementById('reportBtnText').textContent = t.reportBtnText;
    document.getElementById('reportModalTitle').textContent = t.reportModalTitle;
    document.getElementById('reportModalDesc').textContent = t.reportModalDesc;
    document.getElementById('reportCodeLabel').textContent = t.reportCodeLabel;
    document.getElementById('reportNoteLabel').textContent = t.reportNoteLabel;
    document.getElementById('reportSubmitText').textContent = t.reportSubmitText;
...
const debouncedRender = debounce(renderCards, 150);
