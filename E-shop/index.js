...
function addToCart(event) {
    ...
    if (isNaN(price)) {
        console.error('Chyba: Cena produktu nebyla nalezena.');
        return;
    }
...
function renderCart() {
    ...
    if (cart.length === 0) {
        ...
        if (totalPriceElement) totalPriceElement.innerText = '0 Kč';
        if (subtotalPriceElement) subtotalPriceElement.innerText = '0 Kč';
        return;
    }

    const isPaymentPage = window.location.pathname.includes('platebn');

    cart.forEach((item, index) => {
        // Ujistíme se, že cena a množství jsou čísla
        ...
        // Na stránce platby nechceme tlačítka pro změnu množství ani křížek
        const controlsValue = isPaymentPage 
            ? `<span>${itemQuantity} ks</span>` 
            : `
        ...
        itemElement.innerHTML = `
            ...
            <div class="item-info">
                <h4>${item.name}</h4>
                <p>${itemPrice.toLocaleString()} Kč</p>
            </div>
    ...
    const formattedTotal = total.toLocaleString() + ' Kč';
    if (totalPriceElement) totalPriceElement.innerText = formattedTotal;
    if (subtotalPriceElement) subtotalPriceElement.innerText = formattedTotal;
}
